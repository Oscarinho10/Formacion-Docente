...
function calcularEdad(fechaNacimientoStr) {
    // Asegura que solo tomamos la parte de la fecha (sin hora)
    const fechaSolo = fechaNacimientoStr.split(' ')[0]; // "1990-07-08"
    const partes = fechaSolo.split('-'); // ["1990", "07", "08"]

    if (partes.length !== 3) return 0;

    const nacimiento = new Date(partes[0], partes[1] - 1, partes[2]); // yyyy, mm (0-11), dd
    const hoy = new Date();
...
$(document).ready(function () {
    ...
    $(document).on('click', '.verMasBtn', function () {
        ...
        $('#modalEdad').text(edad); // ✅ solo el número
        $('#modalSexo').text(sexo);
