...
SELECT 
  ...
  -- Asistencias válidas
  COUNT(DISTINCT CASE WHEN asi.presente = true THEN asi.id_usuario END) AS asistencias_validas,

  -- Total de sesiones
  COUNT(DISTINCT sa.id_sesion) AS total_sesiones,

  -- Entregas de actividad
  COUNT(DISTINCT ea.id_entrega) AS entrego_actividad

FROM inscripciones i
INNER JOIN usuarios u ON i.id_usuario = u.id_usuario
INNER JOIN actividades_formativas af ON af.id_actividad = i.id_actividad
LEFT JOIN asistencias asi ON asi.id_usuario = u.id_usuario 
                            AND asi.id_sesion IN (SELECT sa.id_sesion FROM sesiones_actividad sa WHERE sa.id_actividad = af.id_actividad)
LEFT JOIN sesiones_actividad sa ON sa.id_actividad = af.id_actividad
LEFT JOIN entregas_actividad ea ON ea.id_inscripcion = i.id_inscripcion
